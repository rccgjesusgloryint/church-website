...
export default function GalleryTopRow({
  setOpen,
  images,
  isLoading,
  fallback,
  ...
  return (
    <div className="flex w-full h-[500px] gap-10 mb-10">
      {isLoading
        ? skeletons.map((_, i) => <ImageSkeleton key={i} aspect="4/3" />)
        : images.map((image, index) => {
            const wide = index === images.length - 1; // last one wide (matches your layout)
            ...
            return (
              ...
              >
                <div className="relative w-full h-full">
                  {/* aspect ratio guard to avoid CLS */}
